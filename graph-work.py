...
class Graph:
    def __init__(self, size):
        # creating an matrix with the values initially set to none
        self.adj_matrix = [[None] * size for _ in range(size)]
        self.size = size
        # Initialize the list to store data
        self.vertex_data = [''] * size
        # methods to add on edges between u and v
    def add_edge(self, u, v, weight):
        if 0 <= u < self.size and 0 <= v < self.size:
            self.adj_matrix[u][v] = weight
            self.adj_matrix[v][u] = weight
            # initialize a list to store data associated with each vertex
    def add_vertex_data(self, vertex, data):
        # to chect if the vertices are within a valid range of the graph
        if 0 <= vertex < self.size:
            self.vertex_data[vertex] = data
            # print out the graph in matrix format  
